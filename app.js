...
passwordSubmit.addEventListener('click', () => {
    if (passwordInput.value === 'john316') {
...
saveProfile.addEventListener('click', () => {
    ...
    userProfile.area = profileArea.value.trim();

    localStorage.setItem('userName', userProfile.name);
    localStorage.setItem('userArea', userProfile.area);

    profileModal.classList.add('hidden');
});

expenseType.addEventListener('change', () => {
    if (expenseType.value === 'driving') {
...
expenseForm.addEventListener('submit', (e) => {
    ...
    const expense = {
        ...
        amount: expenseType.value === 'driving'
            ? (parseFloat(kilometers.value) * 0.26).toFixed(2)
            : parseFloat(amount.value).toFixed(2),
        description: description.value,
        date: dateInput.value,
        kilometers: expenseType.value === 'driving' ? parseFloat(kilometers.value).toFixed(1) : null
    };
    ...
    expensesList.innerHTML = expenses.map(expense => `
        <div class="expense-item" data-id="${expense.id}">
            <div class="expense-item-header">
                <div>
                    <span class="category-badge category-${expense.category}">${getCategoryIcon(expense.category)}</span>
                    <span class="expense-date ml-2">${new Date(expense.date).toLocaleDateString()}</span>
                </div>
                <span class="expense-amount">${expense.amount}€</span>
            </div>
            <p class="expense-description">${expense.description}</p>
            ${expense.kilometers ? `<small class="expense-km">${expense.kilometers} km × 0.26€</small>` : ''}
...
/**
 * Gets the emoji icon for a given expense category
 * @author Santiago
 * @description Returns the appropriate emoji icon to visually represent each expense category type
 * @param {string} category - The category key (food, transport, accommodation, materials, others)
 * @returns {string} The emoji icon for the category
 * @example
 * getCategoryIcon('food'); // Returns '🍽️'
 */
...
generatePDFBtn.addEventListener('click', () => {
    if (expenses.length !== receipts.length) {
        const confirmed = confirm(t('validation_warning', {
